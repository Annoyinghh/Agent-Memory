...
loader.parse(ab, '', (gltf) => {
  ...
  const bb = gltf.scene ? gCloned.boundingBox : null;
  ...
  console.log("Simulating with: vx > 0.74, vy >= -0.36, vy <= 0.24, vz >= -0.46, vz <= 0.0");
  ...
  for (let i = 0; i < count; i++) {
    ...
    const isLeftEar = (vx > 0.74 && vy >= -0.36 && vy <= 0.24 && vz >= -0.46 && vz <= 0.0);
    if (isLeftEar) {
